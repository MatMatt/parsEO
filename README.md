...
The CLI can parse filenames, list available schemas, and assemble filenames from field
dictionaries.

The `assemble` subcommand depends on the `parseo.assembler` module, which ships with the
standard installation. Running `parseo assemble` without this module terminates with:

```
The 'assemble' command requires parseo.assembler, which is part of the standard parseo installation.
```

Reinstall parseo with assembler support or provide a custom `parseo/assembler.py`
implementing `assemble(schema_path, fields)` to enable this command.

The `list-clms-products` subcommand queries the public Copernicus Land Monitoring Service
(CLMS) dataset catalog and prints the available product names to help you discover valid
identifiers for CLMS filename schemas.
